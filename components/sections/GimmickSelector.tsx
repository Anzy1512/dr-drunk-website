...
export function GimmickSelector({ items }: { items: string[] }) {
  ...
  </div><div className="gimmick-options" role="group" aria-label="Explore entertainment ideas">{items.map((item, index) => <button key={item} aria-pressed={selected === index} onClick={() => setSelected(index)}>{item}</button>)}</div><span className="sr-only" role="status">Selected: {items[selected]}</span></div>;
}
