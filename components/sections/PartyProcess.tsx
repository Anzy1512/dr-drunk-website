...
export function PartyProcess() {
  ...
  ].map(([n, title, copy]) => <article key={n}><span aria-hidden="true">✳</span><h3>{title}</h3><p>{copy}</p></article>)}</div><a href="/contact" className="text-link">Let’s get the conversation started <span aria-hidden="true">↗</span></a></section>;
}
