...
export function createLighting(renderer:THREE.WebGLRenderer,scene:THREE.Scene){
  ...
  const key=new THREE.DirectionalLight(0xfff7df,2);key.position.set(-3,5,4);scene.add(key);
  const rim=new THREE.DirectionalLight(0xffad42,1.3);rim.position.set(4,2,-2);scene.add(rim);
