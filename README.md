...
A seven-page brand experience built from the supplied company profile, cocktail collection, wedding storytelling guide, concept boards, and all 50 original photographs.
...
- Gallery: all 50 photographs, progressive loading and an accessible full-image viewer.
...
React, TypeScript, Vinext, Three.js, GSAP/ScrollTrigger and accessible Radix UI controls. Brand colours and typography are defined in `app/globals.css`. Content lives in `lib/brand-content.ts`, `lib/catalogue.ts`, and `lib/gallery-data.json`; page routes live in `app/`.
