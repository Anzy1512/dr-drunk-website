...
The latest logo cleanup uses `logo-transparent.svg`: an embedded copy of the original logo with an SVG alpha filter that removes the dark geometric background. It retains the actual white lettering, cocktail glass, amber earpiece, white stethoscope tube and amber chestpiece. Generated cleanup attempts were rejected for distorted edges and are not published. The header surface behind the logo is transparent; its subtle chestpiece pulse and hover lift use CSS and Theatre's shared glow track.
